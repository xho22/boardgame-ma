...
客户端每 25 秒向 `/ws` 发送一次 `HEARTBEAT`，避免空闲连接被中间网络关闭。连接仍可能因网络切换、浏览器休眠、Cloudflare 边缘维护或服务进程重启而关闭；客户端会以 1、2、4、8、10 秒的上限退避自动重连，并通过已保存的 `playerId` 重新加入原座位。若一个房间所有玩家持续离线 60 秒，服务端会删除该空房间；在宽限期内任一玩家重连会取消清理。
