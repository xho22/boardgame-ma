...
- 所有能力造成的 `move` 都必须在改位置后进入落点结算；因此挪挪、跟班等被动移动停在芭芭雅嘎所在格时同样会绊倒。绊倒恢复只是跳过一次主移动，不是能力触发，不能带动挪挪。
- 跳跳蛙跳过的是除自身外的所有 racer，不区分所属玩家；每跳过 1 名 racer 都是一次独立的能力发生，场上的挪挪须分别移动 1 格。鸡蛋或双胞胎复制该能力时使用相同的移动路径和触发规则。
- 巨婴的同格改写双向生效：其他 racer 落到巨婴格时自己退到巨婴后一格；巨婴落到其他 racer 格时，把全部同格对手推到自己后一格。该改写不是 `move`。若特殊格移动后被推回刚结算的原特殊格，原格效果已消耗，不能形成无限循环。
- 啦啦队长可以支援唯一最后一名，包括自己；自己是唯一最后一名时，先移动 2 格再移动 1 格，然后照常进行主移动。模仿猫独自领先时必须清除复制目标，不能把次领先者误作领先者。
...
- 落点反应先于特殊格效果：若一次落点创建了 `pendingReactions`，特殊格结算器必须暂停；反应完成后再让该格上的每名 racer 各自结算特殊效果。这样 Suckerfish 跟到 `move +3` 格时，会先到该格，再自行前进 3 格，而非被直接传送到目标的最终位置。
